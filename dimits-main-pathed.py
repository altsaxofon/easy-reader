...
class Dimits():
    ...
    def text_2_audio_file(self, text: str, filename: str, directory: str, format: str = 'wav') -> str:
       ...
       """
        if self._get_os() == 'unsupported_machine':
            print("Unsupported machine detected.")
            return
        ...
        #try:
        print(f"Voice ONNX model: {self.voice_onnx}")

        if not os.path.exists(self.voice_onnx):
            print(f"ONNX model file does not exist: {self.voice_onnx}")
            return
        

        out_bin = ttsm('/home/pi/piper/sv_SE-nst-medium.onnx').synthesize(
            text,
            length_scale=1.0,
            noise_scale=1.0,
            noise_w=1.0
        ...
        print(f"Voice ONNX model: {out_bin}")

        if out_bin is None or len(out_bin) == 0:
            print("Synthesize() returned empty output.")
            return

        try:
            with open(filepath, 'wb') as f:
                f.write(out_bin)
                print(f"Audio file written successfully: {filepath}")

            # Verify the file has been written and is non-empty
            if not os.path.exists(filepath):
                print(f"File was not created: {filepath}")
                return

            if os.path.getsize(filepath) == 0:
                print(f"File was created but is empty: {filepath}")
                return
        ...
        #except Exception as e:
        #    print(f"Error during synthesis or file writing: {e}")
        #    return
